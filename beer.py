...
def get_all_beers(product_id_map):
    start_url = "https://api-extern.systembolaget.se/sb-api-ecommerce/v1/productsearch/search"
    products = []
    
    updated_products = []
    try:
        with open("data/beers_updated.json", "r") as f:
            updated_products = json.load(f)
    except FileNotFoundError:
        print("Beers_updated.json not found.")
    except Exception as e:
        print(f"Error with beers_updated.json: {e}")
    ...
    # Different assortment types to search through
    assortment_types = [
        "Säsong",
    ...
    for assortment in assortment_types:
        print(f"\nStarting: {assortment} - Öl (Beer)")
        
        for i in range(1, 500):
            try:
                # Query parameters for beer only
                params = {
                    'page': str(i),
                    'size': '30',
                    'sortBy': 'Score',
                ...
                response = requests.get(start_url, params=params, headers=headers)
    ...
    print(f"\n{'='*60}")
    print(f"SUMMARY:")
    print(f"  Found: {new_product_num} beers")
    print(f"  Delta: {len(found_ids) - len(product_id_map)} beers")
    print(f"  Duplicates: {dup_count} beers")
    print(f"  Deleted: {del_count} beers")
    ...
    with open("data/beers.json", "w", encoding="utf-8") as f:
        json.dump(products, f, indent=2, ensure_ascii=False)
    
    print(f"✓ Wrote {len(products)} beers to data/beers.json")
    ...
    with open("data/beers_updated.json", "w", encoding="utf-8") as f:
        json.dump(updated_products, f, indent=2, ensure_ascii=False)
    
    print(f"✓ Wrote {len(updated_products)} updated beers to data/beers_updated.json")
    print(f"{'='*60}\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("No existing beers file found, starting fresh.")
    
    products = get_all_beers(product_id_map)
    print(f"✓ Complete! Total beers in database: {len(products)}")
